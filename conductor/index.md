# Conductor Index

Navigation hub for all context-driven development artifacts.

## Core Artifacts

- [product.md](product.md) - Product vision and goals
- [product-guidelines.md](product-guidelines.md) - Communication standards
- [tech-stack.md](tech-stack.md) - Technology choices and dependencies
- [workflow.md](workflow.md) - Development practices and quality gates
- [tracks.md](tracks.md) - Work unit registry
- [decisions.md](decisions.md) - Decision log for product and technical decisions

## Source Data

Organized source data from various sources:

### Jira
- [DOM2-6162-epic.md](sources/jira/DOM2-6162-epic.md) - Epic details
- [child-issues.md](sources/jira/child-issues.md) - All child issues

### Documentation
- [master-document.md](sources/docs/master-document.md) - Master document summary
- [dependencies.md](sources/docs/dependencies.md) - Dependencies spreadsheet

### Repository Analysis
- [premium-cart-summary.md](sources/bed/premium-cart-summary.md) - Backend repository analysis
- [premium-cart-anonymous-summary.md](sources/fed/premium-cart-anonymous-summary.md) - Frontend repository analysis

### Team Communication
- [channel-info.md](sources/slack/channel-info.md) - Slack channel details and access
- [README.md](sources/slack/README.md) - Slack integration overview

## Tracks

Individual track directories are in [tracks/](tracks/).

## Agent

- [Agent README](agent/README.md) - Context synchronization agent documentation
- [CHANGELOG.md](CHANGELOG.md) - Live changelog of all changes
- [Reports](reports/) - Daily digest reports

## Context-Driven Development

This structure follows the [Context-Driven Development](https://github.com/wshobson/agents) methodology.
